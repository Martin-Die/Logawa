...
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "if command -v git > /dev/null 2>&1; then echo 'Git est deja installe:'; git --version; else echo 'Installation de Git...'; sudo apt update -qq && sudo apt install -y git && git --version; fi"
...
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "if command -v docker > /dev/null 2>&1; then echo 'Docker est deja installe'; else echo 'Installation de Docker...'; curl -fsSL https://get.docker.com -o get-docker.sh && sudo sh get-docker.sh && sudo usermod -aG docker $USER && rm get-docker.sh; fi"

# 3. Installer Docker Compose si pas deja installe
Write-Host "Verification de Docker Compose..." -ForegroundColor Yellow
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "if docker compose version > /dev/null 2>&1; then echo 'Docker Compose est deja installe'; else echo 'Installation de Docker Compose...'; sudo apt install -y docker-compose-plugin; fi"

# 4. Cloner le depot GitHub
Write-Host "Clonage du depot GitHub..." -ForegroundColor Yellow
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "cd ~ && if [ -d 'Logawa' ]; then echo 'Le dossier Logawa existe deja. Suppression...'; rm -rf Logawa; fi && git clone '$repoUrl' Logawa && cd Logawa && git status"
...
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "cd ~/Logawa && ls -la && echo '' && echo 'Fichiers Docker:' && ls -la *.yml *.yaml 2>/dev/null || echo 'Aucun fichier Docker Compose trouve'"
