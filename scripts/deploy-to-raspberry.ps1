...
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "test -d ~/Logawa/.git"
...
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "docker --version; docker compose version"
if ($LASTEXITCODE -ne 0) {
...
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "cd ~/Logawa; git stash; git pull origin main; git stash pop 2>/dev/null || true"
...
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "cd ~/Logawa; if [ ! -f .env ]; then cp env.example .env; echo 'Fichier .env cree a partir de env.example. Veuillez le configurer.'; fi"
...
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "mkdir -p ~/logawa-logs ~/logawa-backups"
...
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "cd ~/Logawa; docker compose -f docker-compose.raspberry.yml build --no-cache"
...
$logs = ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "cd ~/Logawa; docker compose -f docker-compose.raspberry.yml logs --tail=20"
...
$containerStatus = ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "docker ps --filter name=liko-discord-bot --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'"
...
ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} "df -h"

Write-Host "Deploiement termine !" -ForegroundColor Green
Write-Host "Pour voir les logs en temps reel: ssh -i $SSHKey -p $SSHPort ${User}@${RaspberryIP} 'cd ~/Logawa; docker compose -f docker-compose.raspberry.yml logs -f'" -ForegroundColor Cyan 